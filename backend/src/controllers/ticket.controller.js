...
exports.getTickets = async (req, res) => {
    ...
    try {
        ...
        // Filtro por área/usuario
        if (userArea === 'Personal Operativo') {
            conditions.push(`createdbyuserid::integer = ${paramIndex++}`);
            whereParams.push(userId);
        } else {
            conditions.push(`(assignedtoarea = ${paramIndex++} OR createdbyuserid::integer = ${paramIndex++})`);
        ...
        const ticketsQuery = `
            ...
            FROM "Tickets" t
            JOIN "Users" u ON t.createdbyuserid::integer = u.userid
            ${ticketsWhereClause}
...
exports.getTicketById = async (req, res) => {
    ...
    try {
        ...
        if (userarea === 'Personal Operativo') {
            if (parseInt(ticket.createdbyuserid, 10) !== userid) return res.status(403).json({ msg: 'Acceso denegado.' });
        } else {
            if (ticket.assignedtoarea !== userarea && parseInt(ticket.createdbyuserid, 10) !== userid) {
...
exports.updateTicket = async (req, res) => {
    ...
    try {
        ...
        if (parseInt(ticket.createdbyuserid, 10) !== userid && userrole !== 'Administrador') {
